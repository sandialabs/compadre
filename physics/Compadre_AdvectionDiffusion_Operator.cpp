...
//
//    // GMLS operator
//    GMLS _gmls(_parameters->get<Teuchos::ParameterList>("remap").get<int>("porder"),
//                 2 /* dimension */,
//                 "QR", "STANDARD", "NO_CONSTRAINT");
...
//
//    _gmls->addTargets(TargetOperation::ScalarPointEvaluation);
//    _gmls->setAdditionalEvaluationSitesData(kokkos_quadrature_neighbor_lists_host, kokkos_quadrature_coordinates_host);
//    _gmls->generateAlphas();
//
    ...
    for (int i=0; i<_cells->getCoordsConst()->nLocal(); ++i) {
        ...
        for (size_t j=0; j<cell_particles_all_neighbors[i].size(); ++j) {
            ...
            for (size_t k=0; k<cell_particles_all_neighbors[i].size(); ++k) {
                ...
                //// check distance between j and k (see if they feel each other)
                //auto diff = XYZ(_kokkos_target_coordinates_host(particle_j,0)-_kokkos_target_coordinates_host(particle_k,0),
                //                _kokkos_target_coordinates_host(particle_j,1)-_kokkos_target_coordinates_host(particle_k,1),
                //                _kokkos_target_coordinates_host(particle_j,2)-_kokkos_target_coordinates_host(particle_k,2));

                //auto r = GMLS::EuclideanVectorLength(diff, 2);
                //auto weight = GMLS::Wab(r, _kokkos_epsilons_host(i), WeightingFunctionType::Power, _parameters->get<Teuchos::ParameterList>("remap").get<int>("weighting power"));

                ////if (weight>0 && k_relative_to_j>=0 && j_relative_to_k>=0 && cell_i_for_particle_j>=0 && cell_i_for_particle_k>=0) {
                //if (weight>0 && cell_i_for_particle_j>=0 && cell_i_for_particle_k>=0) {


                //Put the values of alpha in the proper place in the global matrix
                //for (local_index_type k = 0; k < fields[field_one]->nDim(); ++k) {
                            //int extra_site_num_for_j = cell_i_for_particle_j*_weights_ndim+q;
                            //int extra_site_num_for_k = cell_i_for_particle_k*_weights_ndim+q;
