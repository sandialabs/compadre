...
void GMLS_Staggered_PoissonNeumannSources::evaluateRHS(local_index_type field_one, local_index_type field_two, scalar_type time, scalar_type current_timestep_size, scalar_type previous_timestep_size) {
    ...
    for (local_index_type i=0; i<num_neumann; i++) {
        for (local_index_type k=0; k<fields[field_one]->nDim(); k++) {
            ...
            scalar_type b_i = _physics->_neumann_GMLS->getSolutionSetHost()->getAlpha0TensorTo0Tensor(TargetOperation::DivergenceOfVectorPointEvaluation, i, num_neighbors);
...
std::vector<InteractingFields> GMLS_Staggered_PoissonNeumannSources::gatherFieldInteractions() {
