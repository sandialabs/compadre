#include <Compadre_GMLS_StaggeredIntegral_PinnedLaplacian_Operator.hpp>
...
Teuchos::RCP<crs_graph_type> GMLS_StaggeredIntegral_LaplacianPhysics::computeGraph(local_index_type field_one, local_index_type field_two) {
	if (field_two == -1) {
		field_two = field_one;
	}
...
const std::vector<InteractingFields> GMLS_StaggeredIntegral_LaplacianPhysics::gatherFieldInteractions() {
