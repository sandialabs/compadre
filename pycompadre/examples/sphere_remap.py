...
#    def test_edge_tangent_integrated_remap_2d_manifold(self):
#
#        def run(level):
#
#            dataset = Dataset('../../../test_data/grids/sphere_{0}.nc'.format(str(level)), "r", format="NETCDF4")
...
#
#            remap_type  = 1
#            remap_types = (pycompadre.SamplingFunctionals['FaceTangentPointSample'], 
#                          pycompadre.SamplingFunctionals['FaceTangentIntegralSample'])
...
#                        ans_f = f_xyz(rot_xyz/np.linalg.norm(rot_xyz), alt_zonalUnitVector, alt_meridionalUnitVector)
#                        alt_f = np.linalg.solve(R, ans_f)
#                        cross_k_n = np.cross(rot_xyz/np.linalg.norm(rot_xyz), norm_vec)
#                        tang_vec = cross_k_n / np.linalg.norm(cross_k_n)
#                        result += np.dot(alt_f, tang_vec)
#                    else:
#                        # norm_vec is constant for all qp on a great circle
#                        #result += np.dot(f(lat_qp, lon_qp, zonalUnitVector_qp, meridionalUnitVector_qp), norm_vec)*qweights[i]*scaling
#
...
#                        tang_vec_qp = cross_k_n_qp / np.linalg.norm(cross_k_n_qp)
#                        result += np.dot(alt_f, tang_vec_qp)*qweights[i]*scaling
#
#                    edge_length += qweights[i]*scaling
#
...
#        for i in range(0,args.grids):
#            rel_l2_errors.append(run(i))
#        print('edge tangent integrals :: rel_l2_errors', rel_l2_errors)
#
#        rel_l2_rates = []
#        for i in range(1,len(rel_l2_errors)):
#            rel_l2_rates.append(np.log(rel_l2_errors[i]/rel_l2_errors[i-1])/np.log(0.5))
#        print('edge tangent integrals :: rel_l2_rates', rel_l2_rates)
