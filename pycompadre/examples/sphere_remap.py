...
class TestSphereRemap(KokkosTestCase):
    ...
    def test_edge_integrated_remap(self):
        ...
        extra_data = np.zeros(shape=(nEdges, dim*TWO + dim), dtype='f8') # two vertex coordinates plus a normal vector
        edge_points = np.zeros(shape=(nEdges, dim), dtype='f8')

        # need to produce data for the remap (get cell integrated data)
        f = lambda x: [(x[0]/radius)**2 - (x[1]/radius)**2 + (x[1]/radius)*(x[0]/radius) + 55.0, -(x[0]/radius)**2 + (x[1]/radius)**2 + (x[1]/radius)*(x[0]/radius) + 12.0, -3.0*(x[0]/radius)**2 - 5.0*(x[1]/radius)**2 + 12.3*(x[1]/radius)*(x[0]/radius) + 7.6]
        ...
        for edge in range(nEdges):
            ...
            #norm_vec_1 = np.cross(extra_data[edge,0:dim], extra_data[edge,dim:2*dim]) / \
            #             np.linalg.norm(np.cross(extra_data[edge,0:dim], extra_data[edge,dim:2*dim]))
            ...
            #print(norm_vec_1, norm_vec)
            #print(min(np.linalg.norm(norm_vec_1-norm_vec),
            #          np.linalg.norm(norm_vec_1+norm_vec)) \
            #      / np.linalg.norm(norm_vec_1))
            ...
            for i in range(len(qpoints)):
                ...
                # figure out scaling and then integrate 1 on arclengths, compare
                ...
                result += np.dot(f(scaled_transformed_qp), norm_vec)*qweights[i]*scaling
