...
template<typename T>
nb::ndarray<T> create_new_array(const std::vector<size_t> dim_out) {
...
//template<typename T>
//nb::ndarray<T, nb::ndim<1> > create_new_1d_array(size_t dim_out_0) {
//    // 1. Allocate memory and manage ownership using a smart pointer or std::vector
//    auto data_vec = new std::vector<T>(dim_out_0);
//
//    // Optional: Fill the data with some values
//    std::fill(data_vec->begin(), data_vec->end(), 0.0);
//
...
//
//template<typename T>
//nb::ndarray<T, nb::ndim<2> > create_new_2d_array(size_t dim_out_0, size_t dim_out_1) {
//    // 1. Allocate memory and manage ownership using a smart pointer or std::vector
//    auto data_vec = new std::vector<T>(dim_out_0 * dim_out_1);
//
//    // Optional: Fill the data with some values
//    std::fill(data_vec->begin(), data_vec->end(), 0.0);
//
...
//    // 3. Construct the nb::ndarray view over the C++ data
//    // Arguments: data pointer, shape initializer list, owner (the capsule)
//    return nb::ndarray<T, nb::ndim<2> >(
//        data_vec->data(),                 // Pointer to the data
//        {dim_out_0, dim_out_1},           // Shape
//        std::move(owner)                  // Transfer capsule ownership to the ndarray
//    );
//}
//
//template<typename T>
//nb::ndarray<T, nb::ndim<3> > create_new_3d_array(size_t dim_out_0, size_t dim_out_1, size_t dim_out_2) {
//    // 1. Allocate memory and manage ownership using a smart pointer or std::vector
//    auto data_vec = new std::vector<T>(dim_out_0 * dim_out_1 * dim_out_2);
//
//    // Optional: Fill the data with some values
//    std::fill(data_vec->begin(), data_vec->end(), 0.0);
//
...
//    // 3. Construct the nb::ndarray view over the C++ data
//    // Arguments: data pointer, shape initializer list, owner (the capsule)
//    return nb::ndarray<T, nb::ndim<3> >(
//        data_vec->data(),                 // Pointer to the data
//        {dim_out_0, dim_out_1, dim_out_2},           // Shape
//        std::move(owner)                  // Transfer capsule ownership to the ndarray
//    );
//}
...
Kokkos::View<T*, space> convert_np_to_kokkos_1d(nb::ndarray<T> array, 
    ...
    Kokkos::parallel_for(Kokkos::RangePolicy<Kokkos::DefaultHostExecutionSpace>(0, array.shape(0)), [&](int i) {
        ...
        kokkos_array_host(i) = *(double*)((double*)array.data() + i);
...
Kokkos::View<T**, layout, space> convert_np_to_kokkos_2d(nb::ndarray<T> array,
    ...
    Kokkos::parallel_for(Kokkos::RangePolicy<Kokkos::DefaultHostExecutionSpace>(0, array.shape(0)), [&](int i) {
        for (int j=0; j<array.shape(1); ++j) {
            ...
            kokkos_array_host(i,j) = *(double*)((double*)array.data() + i*array.shape(1)+j);
...
template<typename space=Kokkos::HostSpace, typename layout=Kokkos::LayoutRight, typename T>
Kokkos::View<T***, layout, space> convert_np_to_kokkos_3d(nb::ndarray<T> array,
    ...
    Kokkos::parallel_for(Kokkos::RangePolicy<Kokkos::DefaultHostExecutionSpace>(0, array.shape(0)), [&](int i) {
        for (int j=0; j<array.shape(1); ++j) {
            for (int k=0; k<array.shape(2); ++k) {
                ...
                kokkos_array_host(i,j,k) = *(double*)((double*)array.data() + i*array.shape(1)*array.shape(2)+j*array.shape(2)*j+k);
...
struct cknp1d {
    nb::ndarray<typename T::value_type> result;
    cknp1d (T kokkos_array_host) {
        ...
        Kokkos::parallel_for(Kokkos::RangePolicy<Kokkos::DefaultHostExecutionSpace>(0,dim_out_0), [&](int i) {
            ...
            *(double*)((double*)result.data() + i) = kokkos_array_host(i);
        });
        Kokkos::fence();

    }
    nb::ndarray<typename T::value_type> convert() { return result; }
};
    ...
    cknp2d (T kokkos_array_host) {
        ...
        Kokkos::parallel_for(Kokkos::RangePolicy<Kokkos::DefaultHostExecutionSpace>(0,dim_out_0), [&](size_t i) {
            for (size_t j=0; j<dim_out_1; ++j) {
                ...
                *(double*)((double*)result.data() + i*dim_out_1 + j) = kokkos_array_host(i,j);
...
struct cknp3d {
        ...
        Kokkos::parallel_for(Kokkos::RangePolicy<Kokkos::DefaultHostExecutionSpace>(0,dim_out_0), [&](int i) {
            for (int j=0; j<dim_out_1; ++j) {
                for (int k=0; k<dim_out_2; ++k) {
                    ...
                    *(double*)((double*)result.data() + i*result.shape(1)*result.shape(2)+j*result.shape(2)*j+k) = kokkos_array_host(i,j,k);
                }
            }
        });
        Kokkos::fence();
    ...
    nb::ndarray<typename T::value_type, nb::ndim<3> > convert() { return result; }
};
...
template<typename T>
nb::ndarray<typename T::value_type> convert_kokkos_to_np(T kokkos_array_device) {
...
//    }
//
//    void setNeighbors(nb::ndarray<local_index_type> input) {
//
//        if (input.ndim() != 2) {
...
//    }
//
//    nb::ndarray<double> getSourceSites() {
//        compadre_assert_release(gmls_object!=nullptr &&
//                "ParticleHelper used without an internal GMLS object set");
...
//    }
//
//    void setTargetSites(nb::ndarray<double> input) {
//
//        if (input.ndim() != 2) {
//            throw std::runtime_error("Number of dimensions must be two");
//        }
...
//    }
//
//    nb::ndarray<double> getTargetSites() {
//        compadre_assert_release(gmls_object!=nullptr &&
//                "ParticleHelper used without an internal GMLS object set");
...
//    }
//
//    void setWindowSizes(nb::ndarray<double> input) {
//
//        if (input.ndim() != 1) {
...
//    }
// 
//    nb::ndarray<double> getWindowSizes() {
//        compadre_assert_release(gmls_object!=nullptr &&
//                "ParticleHelper used without an internal GMLS object set");
...
//    }
//
//    void setTangentBundle(nb::ndarray<double> input) {
//
//        if (input.ndim() != 3) {
...
//    }
//
//    nb::ndarray<double> getTangentBundle() {
//        compadre_assert_release(gmls_object!=nullptr &&
//                "ParticleHelper used without an internal GMLS object set");
...
//    }
//
//    void setReferenceOutwardNormalDirection(nb::ndarray<double> input, bool use_to_orient_surface = true) {
//
//        if (input.ndim() != 2) {
//            throw std::runtime_error("Number of dimensions must be two");
//        }
...
//    }
//
//    nb::ndarray<double> getReferenceOutwardNormalDirection() {
//        compadre_assert_release(gmls_object!=nullptr &&
//                "ParticleHelper used without an internal GMLS object set");
...
//    }
//
//    void generateKDTree(nb::ndarray<double> input) {
//
//        if (input.ndim() != 2) {
//            throw std::runtime_error("Number of dimensions must be two");
//        }
...
//    }
//
//    void generateNeighborListsFromKNNSearchAndSet(nb::ndarray<double> input, int poly_order, int dimension = 3, double epsilon_multiplier = 1.6, double max_search_radius = 0.0, bool scale_k_neighbor_radius = true, bool scale_num_neighbors = false) {
//
//        compadre_assert_release(gmls_object!=nullptr &&
//                "ParticleHelper used without an internal GMLS object set");
...
//    }
//    
//    void setAdditionalEvaluationSitesData(nb::ndarray<local_index_type> input_indices, nb::ndarray<double> input_coordinates) {
//
...
//    }
//
//    nb::ndarray<double> getPolynomialCoefficients(nb::ndarray<double> input) {
//
...
//                source_data(i, 0) = *((double*)(double*)input.data() + i);
//            });
...
//                    source_data(i, j) = *((double*)(double*)input.data() + i*input.shape(1) + j);
//                }
//            });
//        }
//        Kokkos::fence();
...
//                *(double*)((double*)result.data() + i*dim_out_1 + j) = polynomial_coefficients(i, j);
//            }
//        });
//        Kokkos::fence();
//
...
//    }
// 
//    nb::ndarray<double> applyStencil(const nb::ndarray<double> input, const TargetOperation lro, const SamplingFunctional sro, const int evaluation_site_local_index = 0) const {
// 
...
//    }
//    
//    nb::ndarray<double> applyStencilAllTargetsAllAdditionalEvaluationSites(const nb::ndarray<double> input, const TargetOperation lro, const SamplingFunctional sro) const {
// 
//        // cast numpy data as Kokkos View
//        host_scratch_matrix_left_type source_data((double *) input.data(), input.shape(0), (input.ndim()>1) ? input.shape(1) : 1);
//
...
//    }
//
//    double applyStencilSingleTarget(const nb::ndarray<double> input, const TargetOperation lro, const SamplingFunctional sro, const int evaluation_site_local_index = 0) const {
// 
//        // cast numpy data as Kokkos View
//        host_scratch_matrix_left_type source_data((double *) input.data(), input.shape(0), (input.ndim()>1) ? input.shape(1) : 1);
//
...
////    auto ss      = nb::cast<nb::ndarray<double> >(t[16]);
////    auto k_ss    = convert_np_to_kokkos_2d(ss);
////    auto ts      = nb::cast<nb::ndarray<double> >(t[17]);
////    auto k_ts    = convert_np_to_kokkos_2d(ts);
////    auto ws      = nb::cast<nb::ndarray<double> >(t[18]);
////    auto k_ws    = convert_np_to_kokkos_1d(ws);
////    auto cr      = nb::cast<nb::ndarray<int> >(t[19]);
////    auto k_cr    = convert_np_to_kokkos_1d(cr);
////    auto nnl     = nb::cast<nb::ndarray<int> >(t[20]);
////    auto k_nnl   = convert_np_to_kokkos_1d(nnl);
////    auto a_cr    = nb::cast<nb::ndarray<int> >(t[21]);
////    auto k_a_cr  = convert_np_to_kokkos_1d(a_cr);
////    auto a_nnl   = nb::cast<nb::ndarray<int> >(t[22]);
////    auto k_a_nnl = convert_np_to_kokkos_1d(a_nnl);
////    auto a_es    = nb::cast<nb::ndarray<double> >(t[23]);
...
//    
//    // need to convert from numpy back to kokkos before setting problem data
//    auto ss      = nb::cast<nb::ndarray<double> >(t[7]);
//    auto k_ss    = convert_np_to_kokkos_2d(ss);
//    auto ts      = nb::cast<nb::ndarray<double> >(t[8]);
//    auto k_ts    = convert_np_to_kokkos_2d(ts);
//    auto ws      = nb::cast<nb::ndarray<double> >(t[9]);
//    auto k_ws    = convert_np_to_kokkos_1d(ws);
//    auto cr      = nb::cast<nb::ndarray<int> >(t[10]);
//    auto k_cr    = convert_np_to_kokkos_1d(cr);
//    auto nnl     = nb::cast<nb::ndarray<int> >(t[11]);
//    auto k_nnl   = convert_np_to_kokkos_1d(nnl);
//    auto a_cr    = nb::cast<nb::ndarray<int> >(t[12]);
//    auto k_a_cr  = convert_np_to_kokkos_1d(a_cr);
//    auto a_nnl   = nb::cast<nb::ndarray<int> >(t[13]);
//    auto k_a_nnl = convert_np_to_kokkos_1d(a_nnl);
//    auto a_es    = nb::cast<nb::ndarray<double> >(t[14]);
//    auto k_a_es  = convert_np_to_kokkos_2d(a_es);
//    
//    gmls.setAdditionalEvaluationSitesData(k_a_cr, k_a_nnl, k_a_es);
//    gmls.setProblemData(k_cr, k_nnl, k_ss, k_ts, k_ws);
...
//// This function acts as the factory during unpickling
//Animal factory_function(std::string name, int age) {
//    return Animal(name, age);
//}
//
//std::tuple<nb::object, std::tuple<std::string, int>> animal_reduce(const Animal& a) {
//    // 1. The factory function pointer (cast to an nb::object/capsule)
//    nb::object factory = nb::cpp_function(&factory_function);
//
//    // 2. The arguments needed for the factory function
//    std::tuple<std::string, int> args = std::make_tuple(a.name, a.age);
//
//    // Return the required tuple structure: (callable, arguments_tuple)
//    return std::make_tuple(factory, args);
//}



//auto std::tuple<nb::object, std::tuple<std::string, int>> animal_reduce(const Animal& a) {

//.def("__reduce__", &animal_reduce);
//
//nb::dict get_gmls_state(const GMLS& gmls) {
//    nb::dict state;
//    state["name"] = a.name;
//    state["age"] = a.age;
//    return state;
...
//void set_gmls_state(GMLS& a, std::tuple<std::string, int> state) {
//    .def(nb::pickle(
...
// 
//            // get all relevant details from GMLS class
//            return nb::make_tuple(rs, psf, dsf, po, gdim, dst, pt, ct, cpo, 
//                                  wt, wp0, wp1, mwt, mwp0, mwp1, lro_list,
//                                  np_ss, np_ts, np_ws, np_cr, np_nnl,
//                                  np_a_cr, np_a_nnl, np_a_es);
//
...
//            // reinstantiate with details
//            // no need to keep solutions
//            GMLS gmls(nb::cast<const ReconstructionSpace>(t[0]),
...
https://github.com/sandialabs/compadre/blob/master/pycompadre/pycompadre.cpp
    ...
    //.def("__setstate__", [](nb::tuple t) { // __setstate__
    //        if (t.size() != 6)
    //            throw std::runtime_error("Invalid state!");
    //        SamplingFunctional sf(nb::cast<int>(t[0]), nb::cast<int>(t[1]), nb::cast<bool>(t[2]),
    ...
    //.def("generateKDTree", static_cast<void (ParticleHelper::*)(nb::ndarray<double>)>(&ParticleHelper::generateKDTree))
    ...
    //.def("setTargetExtraData", [] (GMLS* gmls, nb::ndarray<double> extra_data) {
    //    if (extra_data.ndim() != 2) {
    //        throw std::runtime_error("Number of dimensions must be two");
    //    }
    //    gmls->setTargetExtraData(convert_np_to_kokkos_2d(extra_data));
    //})
    //.def("setSourceExtraData", [] (GMLS* gmls, nb::ndarray<double> extra_data) {
    //    if (extra_data.ndim() != 2) {
    //        throw std::runtime_error("Number of dimensions must be two");
    //    }
