...
//		 	double end_time = parameters->get<Teuchos::ParameterList>("time").get<double>("t_end");
		    ...
		    auto integral_vel_function = Compadre::CosT(parameters->get<Teuchos::ParameterList>("time").get<double>("t_end"),-1);
		    auto integral_h_function = Compadre::CosT(parameters->get<Teuchos::ParameterList>("time").get<double>("t_end"),-1);

			for(int j =0; j<coords->nLocal(); j++){

				xyz_type xyz = coords->getLocalCoords(j, true /* use_halo */, true);
				xyz_type exact = integral_vel_function.evalScalar(xyz,parameters->get<Teuchos::ParameterList>("time").get<double>("t_end"))+1;
				ST exact_height = integral_h_function.evalScalar(xyz,parameters->get<Teuchos::ParameterList>("time").get<double>("t_end"))+1;
