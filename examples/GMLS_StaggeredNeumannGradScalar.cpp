...
// otherwise, Views may be deallocating when we call Kokkos::finalize() later
{
    ...
    int constraint_type = 1; // Using Neumann Gradient Scalar here
    if (argc >= 7) {
    ...
    int solver_type = 0; // SVD is required to solve GMLS using staggered scheme with constraints
    if (argc >= 5) {
    ...
    TargetOperation lro;
    lro = DivergenceOfVectorPointEvaluation;
    ...
    for (int i=0; i<number_target_coords; i++) {
        ...
        double adjusted_divergence_scalar = GMLS_Divergence_Scalar + b_i_scalar*g;

        // check divergence
        if(std::abs(actual_Divergence - adjusted_divergence_scalar) > failure_tolerance) {
            all_passed = false;
            std::cout << i << " Failed Divergence on SCALAR basis by: " << std::abs(actual_Divergence - adjusted_divergence_scalar) << std::endl;
            std::cout << i << " GMLS " << GMLS_Divergence_Scalar << " adjusted " << adjusted_divergence_scalar << " actual " << actual_Divergence << std::endl;
        }

        double b_i_vector = vector_basis_gmls.getAlpha0TensorTo0Tensor(DivergenceOfVectorPointEvaluation, i, num_neigh_i);
        double adjusted_divergence_vector = GMLS_Divergence_Vector + b_i_vector*g;
        if(std::abs(actual_Divergence - adjusted_divergence_vector) > failure_tolerance) {
            all_passed = false;
            std::cout << i << " Failed Divergence on VECTOR basis by: " << std::abs(actual_Divergence - adjusted_divergence_vector) << std::endl;
            std::cout << i << " GMLS " << GMLS_Divergence_Vector << " adjusted " << adjusted_divergence_vector << " actual " << actual_Divergence << std::endl;
        }
    }
