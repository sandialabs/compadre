...
//			particles->getFieldManager()->printAllFields(std::cout);
		...
		}
		{
			...
			fm.read();
		 	...
		 	// check if each flag read in as i
		 	for (int i=0; i<50; i++) {
		 		std::ostringstream msg;
		 		msg << "bc_id read in wrong value for " << i << " of " << particles->getFlag(i) << " instead of " << i;
		 		// accomodate for removing particles 50 through 200 from each processor
		 		TEUCHOS_TEST_FOR_EXCEPT_MSG(particles->getFlag(i)-i != 0, msg.str());
		 	}
		 	// check if each flag read in as i
		 	for (int i=50; i<sphCoords->nLocal(); i++) {
		 		std::ostringstream msg;
		 		msg << "bc_id read in wrong value for " << i << " of " << particles->getFlag(i)-151 << " instead of " << i;
		 		// accomodate for removing particles 50 through 200 from each processor
		 		TEUCHOS_TEST_FOR_EXCEPT_MSG(particles->getFlag(i)-151-i != 0, msg.str());
		 	}
