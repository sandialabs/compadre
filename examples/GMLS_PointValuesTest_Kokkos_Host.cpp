...
//            }
    ...
    for (int i=0; i<number_target_coords; i++) {
        ...
        double GMLS_value = my_GMLS.applyAlphasToData(sampling_data, ScalarPointEvaluation, i, 0, 0, 0, 0);
        ...
        double GMLS_Laplacian = my_GMLS.applyAlphasToData(sampling_data, LaplacianOfScalarPointEvaluation, i, 0, 0, 0, 0);
        ...
        double GMLS_GradX = my_GMLS.applyAlphasToData(sampling_data, GradientOfScalarPointEvaluation, i, 0, 0, 0, 0);
        ...
        double GMLS_GradY = (dimension>1) ? my_GMLS.applyAlphasToData(sampling_data, GradientOfScalarPointEvaluation, i, 1, 0, 0, 0) : 0;
        ...
        double GMLS_GradZ = (dimension>2) ? my_GMLS.applyAlphasToData(sampling_data, GradientOfScalarPointEvaluation, i, 2, 0, 0, 0) : 0;
        ...
        double GMLS_Divergence = my_GMLS.applyAlphasToData(sampling_data, DivergenceOfVectorPointEvaluation, i, 0, 0, 0, 0);
        if (dimension>1) GMLS_Divergence += my_GMLS.applyAlphasToData(sampling_data, DivergenceOfVectorPointEvaluation, i, 0, 0, 1, 0);
        if (dimension>2) GMLS_Divergence += my_GMLS.applyAlphasToData(sampling_data, DivergenceOfVectorPointEvaluation, i, 0, 0, 2, 0);
        ...
        if (dimension>1) {
            for (int j=0; j<dimension; ++j) {
                GMLS_CurlX += my_GMLS.applyAlphasToData(divergence_sampling_data, j, CurlOfVectorPointEvaluation, i, 0, 0, j, 0);
                GMLS_CurlY += my_GMLS.applyAlphasToData(divergence_sampling_data, j, CurlOfVectorPointEvaluation, i, 1, 0, j, 0);
            }
        } 

        if (dimension>2) {
            for (int j=0; j<dimension; ++j) {
                GMLS_CurlZ += my_GMLS.applyAlphasToData(divergence_sampling_data, j, CurlOfVectorPointEvaluation, i, 2, 0, j, 0);
            }
