...
// otherwise, Views may be deallocating when we call Kokkos::finalize() later
{
    ...
    Kokkos::View<double**, Kokkos::DefaultExecutionSpace> additional_target_coords_device ("additional target coordinates", number_target_coords, 3);
    ...
    Kokkos::View<int**, Kokkos::DefaultExecutionSpace> additional_target_indices_device ("additional target indices", number_target_coords, 2);
    ...
    for(int i=0; i<number_target_coords; i++){
        ...
        for (int j=0; j<dimension; ++j) {
            additional_target_coords(i,j) = target_coords(i,j) + 1e-7;
        }
        
        // set list of indices for extra evaluations
        additional_target_indices(i,0) = 1;
        additional_target_indices(i,1) = i;
