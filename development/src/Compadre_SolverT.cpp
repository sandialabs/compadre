		...
		_comm(comm)
{
	...
	for (local_index_type i=0; i<A.size(); i++) {
		...
		for (local_index_type j=0; j<A[0].size(); j++) {
			if (!(A[i][j].is_null())) {
				_A->setBlock(i, j, Thyra::tpetraLinearOp<scalar_type>(
						Thyra::tpetraVectorSpace<scalar_type>(A[i][j]->getDomainMap()),
						Thyra::tpetraVectorSpace<scalar_type>(A[i][j]->getRangeMap()),
						(Teuchos::RCP<op_type>)(A[i][j]))
						);
				_A_tpetra[i][j] = A[i][j];
			}
		}
	...
	for (local_index_type i=0; i<b.size(); i++) {
		...
		_b[i] = Thyra::tpetraVector(domain, Teuchos::rcp_static_cast<vec_type>(b[i]));
		_b_tpetra[i] = b[i];
	}
}
