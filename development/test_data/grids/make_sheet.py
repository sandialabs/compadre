...
for key, h in enumerate(h_all):
    ...
    flagged_layers_left_and_right = .2*circumference_points
    ...
    for vertical in range(z_layer):
        for horizontal in range(circumference_points):
            ...
            x = horizontal*h + h*rand_pert_x
            y = 0
     
            baseline_vert = 0 if vertical==0 else (vertical-1) * (height / (z_layer-1))
            z = vertical*(height / (z_layer-1))
            if (vertical-flagged_layers_top_and_bottom < 0) or (vertical - (z_layer-1 - flagged_layers_top_and_bottom) > 0) or (horizontal-flagged_layers_left_and_right < 0) or (horizontal - (circumference_points-1 - flagged_layers_left_and_right) > 0):
