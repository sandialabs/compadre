...
protected:
    ...
    //! (dimensions x rank of tensor of output )^2 for each target functional (device)
    Kokkos::View<int*, layout_type> _lro_output_tile_size; 
    //! (dimensions x rank of tensor of output )^2 for each target functional (host)
    Kokkos::View<int*, layout_type>::HostMirror _host_lro_output_tile_size; // size of this output dependent on tensor rank

    //! (dimensions x rank of tensor of output )^2 for each sampling functional (device)
    Kokkos::View<int*, layout_type> _lro_input_tile_size; 
    //! (dimensions x rank of tensor of output )^2 for each sampling functional (host)
...
public:
    ...
    //! Returns the dimension of the GMLS problem, set at instantiation
