...
def approximate(porder, wpower, wtype, epsilon_multiplier, attempt_center_about_coord):
    ...
    Z_pred = gmls_helper.applyStencil(Z.ravel(order='C'), pycompadre.TargetOperation.ScalarPointEvaluation)
    ...
    return np.reshape(Z_pred, newshape=(len(x_pred), len(y_pred)))

# get initial data for plotting
#z_pred, computed_answer, center_about_extra_idx, center_about_extra_coord = approximate(polynomial_order, 3, 'power', epsilon_multiplier, 1.2)
Z_pred = approximate(polynomial_order, 3, 'power', epsilon_multiplier, np.atleast_2d([1.0,1.0]))
# plot initial data
fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')
plt.subplots_adjust(left=0.25, bottom=0.25)
#d, = plt.plot(x_pred, y_pred, c='#0000FF', linewidth=4, zorder=1)
d = ax.plot_surface(X_pred, Y_pred, Z_pred, zorder=1)
p = ax.scatter(X.ravel(order='C'), Y.ravel(order='C'), Z.ravel(order='C'), c='#000000', marker='D', zorder=2)
